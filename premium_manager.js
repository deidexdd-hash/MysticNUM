...
class PremiumManager {
    ...
    renderAncestral() {
        ...
        const programs = this.ancestralSystem.analyzeAncestralPrograms();
        const tree = this.ancestralSystem.generateFamilyTreeVisualization();
        const stats = this.ancestralSystem.getAncestralStats(programs);
        
        let html = '';

        // Родовое древо
        html += `
            <div class="ancestral-tree fade-in-up">
        ...
        tree.generations.forEach((gen, idx) => {
            html += `
                <div class="tree-generation" style="animation-delay: ${idx * 0.1}s;">
                    <div class="tree-node">
                        <div class="tree-node-level">Поколение ${gen.level}</div>
                        <div class="tree-node-name">${gen.name}</div>
                        <div class="tree-node-count">${gen.members} ${this.pluralize(gen.members, 'человек', 'человека', 'человек')}</div>
                    </div>
                </div>
            `;
        });
        ...
        html += `
                <div style="text-align: center; margin-top: 30px; padding: 20px; background: rgba(99, 102, 241, 0.1); border-radius: 12px;">
                    <strong style="color: var(--accent-primary);">${tree.keyMessage}</strong>
                </div>
            </div>
        `;
        ...
        // Статистика
        html += `
            <div class="plan-stats-card" style="margin-top: 30px;">
                <h3 style="margin-bottom: 20px;">📊 Анализ Рода</h3>
                <div class="plan-stats-grid">
                    <div class="plan-stat-item">
                        <div class="plan-stat-value">${stats.totalPrograms}</div>
                        <div class="plan-stat-label">Программ обнаружено</div>
                    </div>
                    <div class="plan-stat-item">
                        <div class="plan-stat-value" style="color: var(--danger);">${stats.criticalIssues}</div>
                        <div class="plan-stat-label">Критичных</div>
                    </div>
                    <div class="plan-stat-item">
                        <div class="plan-stat-value">${stats.karmicDebts}</div>
                        <div class="plan-stat-label">Кармических долгов</div>
                    </div>
                    <div class="plan-stat-item">
                        <div class="plan-stat-value">${stats.estimatedWorkDuration}</div>
                        <div class="plan-stat-label">Дней работы</div>
                    </div>
                </div>
            </div>
        `;
        ...
        html += `<h3 style="margin: 30px 0 20px 0;">⚡ Обнаруженные программы</h3>`;
        
        programs.detected.forEach((program, idx) => {
            html += `
                <div class="program-card severity-${program.severity}" style="animation: fadeInUp 0.5s ease ${idx * 0.1}s backwards;">
                    <span class="program-type ${program.type}">${this.getProgramTypeLabel(program.type)}</span>
                    <h4 class="program-title">${program.title}</h4>
                    <p class="program-description">${program.description}</p>
                    ${program.signs ? `
                        <div class="program-signs">
                            <div class="program-signs-title">Признаки:</div>
                            <div class="program-signs-list">
                                ${program.signs.map(sign => `<span class="sign-tag">${sign}</span>`).join('')}
                            </div>
                        </div>
                    ` : ''}
                    <div style="padding: 12px; background: rgba(16, 185, 129, 0.1); border-radius: 8px; font-size: 14px;">
                        <strong style="color: var(--success);">Исцеление:</strong> ${program.healing}
                    </div>
                </div>
            `;
        });
        ...
        // Рекомендации
        if (programs.recommendations.length > 0) {
            html += `
                <h3 style="margin: 30px 0 20px 0;">💡 План работы с Родом</h3>
                <div class="recommendations-widget">
            `;
            
            programs.recommendations.forEach((rec, idx) => {
                html += `
                    <div class="recommendation-item">
                        <div class="recommendation-icon">${idx + 1}️⃣</div>
                        <div class="recommendation-content">
                            <h4 style="margin: 0 0 8px 0; color: var(--text-primary);">${rec.title}</h4>
                            <p class="recommendation-text">${rec.description}</p>
                            <div style="font-size: 13px; color: var(--text-muted);">
                                ⏱️ Длительность: ${rec.duration}
                            </div>
                        </div>
                    </div>
                `;
